...
public class CoinFlipListGUI extends InventoryGUI {
   ...
   public void decorate(Player player) {
      if (player != null && player.isOnline()) {
         ...
         if (inventory != null) {
            ...
            String fillerMaterialName = this.plugin.getGUIConfig().getString("list-gui.filler.material",
            ...
            List<Integer> gameSlotsList = this.plugin.getGuiHelper().parseSlotList(this.plugin.getGUIConfig(),
                  "list-gui.game-slots");
            int[] gameSlots = gameSlotsList.isEmpty()
                  ? new int[] { 10, 11, 12, 13, 14, 15, 16, 19, 20, 21, 22, 23, 24, 25, 28, 29, 30, 31, 32, 33, 34 }
                  : gameSlotsList.stream().mapToInt(i -> i != null ? i : -1)
            ...
            String leaderboardMaterialName = this.plugin.getGUIConfig().getString("list-gui.leaderboard.material",
                  "GOLD_INGOT");
            Material leaderboardMaterial = this.parseMaterial(leaderboardMaterialName, Material.GOLD_INGOT);
            String leaderboardTitle = this.plugin.getGUIConfig().getString("list-gui.leaderboard.title",
                  "&r&6&lLeaderboard");
            ...
            this.addButton(
                  ...
                  new InventoryButton()
                        .creator(
                              p -> this.createLeaderboardButton(leaderboardMaterial, leaderboardTitle, leaderboardLore))
                        .consumer(event -> {
                              ...
                              if (this.viewer != null && this.viewer.isOnline()) {
                                 ...
                                 FoliaScheduler.runTaskLater(this.plugin, this.viewer, () -> {
                                    if (this.viewer != null && this.viewer.isOnline()) {
                                       ...
                                       this.logGuiDebug(
                                             "Opening leaderboard GUI from main menu for " + this.viewer.getName());
                                       this.plugin.getGuiManager().openGUI(new LeaderboardGUI(this.plugin, this.viewer),
                                             this.viewer);
                                    }
                                 }, 2L);
                              }
            ...
            String historyTitle = this.plugin.getGUIConfig().getString("list-gui.history.title",
                  "&r&6&lYour CoinFlip History");
            List<String> historyLore = this.plugin.getGUIConfig().getStringList("list-gui.history.lore");
            this.addButton(
                  historySlot,
                  new InventoryButton()
                        .creator(p -> this.createHistoryButton(historyMaterial, historyTitle, historyLore))
                        .consumer(event -> {
                              ...
                              if (this.viewer != null && this.viewer.isOnline()) {
                                 ...
                                 FoliaScheduler.runTaskLater(this.plugin, this.viewer, () -> {
                                    if (this.viewer != null && this.viewer.isOnline()) {
                                       ...
                                       this.logGuiDebug("Opening history GUI (page 1) from main menu for "
                                             + this.viewer.getName());
                                       this.plugin.getGuiManager()
                                             .openGUI(new CoinFlipHistoryGUI(this.plugin, this.viewer, 1), this.viewer);
                                    }
                                 }, 2L);
                              }
                           });
            ...
            this.addButton(createSlot, new InventoryButton()
                  .creator(p -> this.createCreateButton(createMaterial, createTitle, createLore)).consumer(event -> {
                        ...
                        if (this.viewer != null && this.viewer.isOnline()) {
                           ...
                           FoliaScheduler.runTaskLater(this.plugin, this.viewer, () -> {
                              if (this.viewer != null && this.viewer.isOnline()) {
                                 ...
                                 this.plugin.getGuiManager().openGUI(new CreateCoinFlipGUI(this.plugin, this.viewer),
                                       this.viewer);
                              }
                           }, 2L);
                        }
            ...
            this.addButton(
                  filterSlot,
                  new InventoryButton()
                        .creator(p -> this.createFilterButton(filterMaterial, filterTitle, filterLoreTemplate,
                              this.filterType))
                        .consumer(event -> {
                           ...
                           Player clicker = (Player) event.getWhoClicked();
                           this.logGuiDebug(String.format("[Filter:updating] player=%s filter=%s->%s",
                                 clicker.getName(), this.filterType, finalNextFilter));
                           ...
                           FoliaScheduler.runTask(this.plugin, this.viewer, () -> {
                              if (this.viewer != null && this.viewer.isOnline()) {
                                 ...
                                 this.logGuiDebug(
                                       "Applying filter '" + filterToApply + "' for " + this.viewer.getName());
            ...
            if (hasPrevPage) {
               ...
               String prevText = this.plugin
                     .getAdventureHelper()
                     .parseToLegacy(this.plugin.getGUIConfig().getString("list-gui.navigation.previous.text",
                           "&cPrevious &8(&e<page>&8)"), navPlaceholders);
               String prevMaterialName = this.plugin.getGUIConfig().getString("list-gui.navigation.previous.material",
                     "PLAYER_HEAD");
               ...
               this.addButton(
                     prevSlot,
                     new InventoryButton()
                           .creator(p -> this.createNavigationItem(prevMaterial, prevText, prevBase64,
                                 "list-gui.navigation.previous"))
                           .consumer(event -> {
                              Player clicker = (Player) event.getWhoClicked();
                              this.plugin.getSoundHelper().playSound(clicker, "gui.page-change");
                              this.logGuiDebug(String.format("[PrevPage:updating] player=%s page=%d->%d",
                                    clicker.getName(), this.page, this.page - 1));
                              this.page--;
                              FoliaScheduler.runTask(this.plugin, this.viewer, () -> {
                                 if (this.viewer != null && this.viewer.isOnline()) {
                                    if (this.hasChestInventoryOpen(this.viewer)) {
            ...
            if (hasNextPage) {
               ...
               String nextText = this.plugin
                     .getAdventureHelper()
                     .parseToLegacy(this.plugin.getGUIConfig().getString("list-gui.navigation.next.text",
                           "&aNext Page &8(&e<page>&8)"), navPlaceholders);
               String nextMaterialName = this.plugin.getGUIConfig().getString("list-gui.navigation.next.material",
                     "PLAYER_HEAD");
               ...
               this.addButton(
                     nextSlot,
                     new InventoryButton()
                           .creator(p -> this.createNavigationItem(nextMaterial, nextText, nextBase64,
                                 "list-gui.navigation.next"))
                           .consumer(event -> {
                              Player clicker = (Player) event.getWhoClicked();
                              this.plugin.getSoundHelper().playSound(clicker, "gui.page-change");
                              this.logGuiDebug(String.format("[NextPage:updating] player=%s page=%d->%d",
                                    clicker.getName(), this.page, this.page + 1));
                              this.page++;
                              FoliaScheduler.runTask(this.plugin, this.viewer, () -> {
                                 if (this.viewer != null && this.viewer.isOnline()) {
      ...
      if (this.plugin != null && message != null) {
         try {
            if (this.plugin.getDebugManager() != null) {
               this.plugin.getDebugManager().debug(DebugManager.Category.GUI, DebugManager.Level.INFO,
                     "[CoinFlipListGUI] " + message);
            }
         ...
         this.refreshTaskId = FoliaScheduler.runTaskTimer(this.plugin, this.viewer, () -> {
            if (this.viewer != null && this.viewer.isOnline()) {
               try {
                  ...
                  String listGuiTitleTemplate = this.plugin.getGUIConfig().getString("list-gui.title",
                        "&8UltraCoinFlip &8[&7Page <page>&8]");
                  String strippedTemplate = ChatColor.stripColor(
                        ChatColor.translateAlternateColorCodes('&', listGuiTitleTemplate.replace("<page>", "")));
                  ...
                  if (filteredGames.size() != this.lastDisplayedGamesCount) {
                     this.plugin.getGuiManager().openGUI(
                           new CoinFlipListGUI(this.plugin, this.viewer, currentPage, currentFilterType), this.viewer);
                     return;
                  }
                  ...
                  if (this.plugin.getDebugManager() != null
                     ...
                     this.plugin.getDebugManager().verbose(DebugManager.Category.GUI,
                           "Error in auto-refresh task: " + var10.getMessage());
                  }
         ...
         if (this.plugin.getDebugManager() != null
            ...
            this.plugin
                  ...
                  .verbose(DebugManager.Category.GUI,
                        "[CoinFlipListGUI] refreshGUI() called for " + this.viewer.getName() + ", page=" + this.page);
         }
                     ...
                     try {
                           ...
                           if (this.plugin.getDebugManager() != null
                              ...
                              this.plugin
                                    ...
                                    .verbose(
                                          ...
                                          "[CoinFlipListGUI] Skipping refresh for " + this.viewer.getName()
                                                + " (different GUI open: '" + strippedCurrent + "')");
                           }
                        } else {
                           if (this.plugin.getDebugManager() != null
                                 && this.plugin.getDebugManager().isCategoryEnabled(DebugManager.Category.GUI)) {
                              this.plugin
                                    ...
                                    .verbose(
                                          DebugManager.Category.GUI,
                                          "[CoinFlipListGUI] Refreshing GUI for " + this.viewer.getName()
                                                + " (title matches: '" + strippedCurrent + "')");
                           }

                           this.plugin.getGuiManager().openGUI(
                                 new CoinFlipListGUI(this.plugin, this.viewer, this.page, this.filterType),
                                 this.viewer);
                        }
                        ...
                        if (this.plugin.getDebugManager() != null
                           ...
                           this.plugin
                                 ...
                                 .verbose(
                                       DebugManager.Category.GUI,
                                       "[CoinFlipListGUI] Error checking inventory for " + this.viewer.getName() + ": "
                                             + var6.getMessage() + ", skipping refresh");
                        }
                     }
                  }
               });
   ...
   public static void refreshAllViewers(KStudio plugin) {
      if (plugin != null && plugin.getGuiManager() != null) {
         if (plugin.getDebugManager() != null
               && plugin.getDebugManager().isCategoryEnabled(DebugManager.Category.GUI)) {
         ...
         if (plugin.getDebugManager() != null
               && plugin.getDebugManager().isCategoryEnabled(DebugManager.Category.GUI)) {
            plugin.getDebugManager().verbose(DebugManager.Category.GUI,
                  "[CoinFlipListGUI] refreshAllViewers() found " + count[0] + " handlers");
         }
      }
   }
   ...
   private ItemStack createGameItem(CoinFlipGame game) {
      if (game != null && game.getHost() != null) {
         ...
         if (game.getCurrencyType() == CoinFlipGame.CurrencyType.MONEY) {
            ...
            typeDisplayName = this.plugin.getCurrencyManager().getDisplayName(game.getCurrencyType(),
                  game.getCurrencyId());
         } else {
            typeDisplayName = this.plugin.getCurrencyManager().getDisplayName(game.getCurrencyType(),
                  game.getCurrencyId());
         ...
         UUID hostUuid = game.getHostUuid();
         PlayerStats hostStats = hostUuid != null ? this.plugin.getCoinFlipManager().getStats(hostUuid)
               : new PlayerStats();
         String winRate = hostStats.getTotalGames() > 0 ? String.format("%.1f", hostStats.getWinPercentage()) : "N/A";
         double taxRate = this.plugin.getTaxRateCalculator().calculateTaxRate(this.viewer, game.getAmount(),
               game.getCurrencyType(), game.getCurrencyId());
         ...
         String clickMessageText = this.plugin.getGUIConfig().getString("game-item.lore.click-message",
               "Click to accept");
         String clickMessage = clickColor + clickMessageText;
         String cancelMessageText = this.plugin.getGUIConfig().getString("game-item.lore.cancel-message",
         ...
         boolean isOwnGame = this.viewer != null && game.getHostUuid() != null
               && this.viewer.getUniqueId().equals(game.getHostUuid());
         ...
         String displayNameTemplate = this.plugin.getGUIConfig().getString("game-item.display-name",
               "&r&6&l<player> &f- &e<amount><symbol>");
         Player placeholderSource = hostPlayer != null ? hostPlayer : this.viewer;
         String displayName = this.plugin.getAdventureHelper().parseToLegacy(displayNameTemplate, displayPlaceholders,
               placeholderSource);
         ...
         synchronized (CoinFlipListGUI.class) {
            ...
            if (guiConfig == null) {
               ...
               loreTemplate = cachedGameItemLoreTemplate != null ? new ArrayList<>(cachedGameItemLoreTemplate)
                     : new ArrayList<>();
            }
         }
      ...
      if (stats == null) {
         ...
         synchronized (CoinFlipListGUI.class) {
            ...
            if (guiConfig == null) {
               ...
               loreTemplate = (List<String>) (cachedStatsItemLoreTemplate != null ? cachedStatsItemLoreTemplate
                     : new ArrayList<>());
            }
         }
         ...
         if (MaterialHelper.isPlayerHead(material)) {
            ...
            return this.plugin.getGuiHelper().createPlayerHead(material, headPlayer, base64Texture, usePlayerSkin,
                  parsedTitle, lore, glowing, customModelData);
         } else {
            ItemStack item = new ItemStack(material);
            ItemMeta meta = item.getItemMeta();
            if (meta != null) {
      ...
      if (MaterialHelper.isPlayerHead(material)) {
         if (base64Texture != null && !base64Texture.isEmpty()) {
            ItemStack item = this.plugin
                  .getGuiHelper()
                  .createPlayerHead(material, null, base64Texture, false, name != null ? name : "", new ArrayList(),
                        glowing, customModelData);
   ...
   private ItemStack createFilterButton(Material material, String title, List<String> loreTemplate,
      ...
      if (meta == null) {
         ...
         switch (currentFilter) {
            case "TIME_NEWEST":
               filterDisplayName = this.plugin.getGUIConfig().getString("list-gui.filter.filter-names.time-newest",
                     "Time (Newest)");
               break;
            case "AMOUNT_ASCENDING":
               filterDisplayName = this.plugin.getGUIConfig().getString("list-gui.filter.filter-names.amount-ascending",
                     "Amount (Low to High)");
               break;
            case "AMOUNT_DESCENDING":
               filterDisplayName = this.plugin.getGUIConfig()
                     .getString("list-gui.filter.filter-names.amount-descending", "Amount (High to Low)");
               break;
            default:
   ...
   private void updateFilterButton() {
      ...
      if (inventory != null) {
         ...
         if (filterSlot >= 0 && filterSlot < inventory.getSize()) {
            ...
            String filterTitle = this.plugin.getGUIConfig().getString("list-gui.filter.title", "&r&6&lFilter Games");
            List<String> filterLoreTemplate = this.plugin.getGUIConfig().getStringList("list-gui.filter.lore");
            ItemStack filterItem = this.createFilterButton(filterMaterial, filterTitle, filterLoreTemplate,
                  this.filterType);
   ...
   private void handleGameClick(InventoryClickEvent event, Player clicker, CoinFlipGame game) {
      if (event.getClick() == ClickType.RIGHT) {
            ...
            if (this.plugin.getCoinFlipManager().isOnRefundCooldown(clicker.getUniqueId())) {
               ...
               if (this.plugin.getPlayerSettingsManager().isSettingEnabled(clicker, "message-error")) {
                  ...
                  String message = this.plugin.getMessage("prefix")
                        + " "
                        + this.plugin.getMessage("command.cooldown-wait").replace("<seconds>",
               ...
               if (!this.plugin.getCoinFlipManager().refundGame(clicker, gameIdToRemove)) {
                  if (this.plugin.getPlayerSettingsManager().isSettingEnabled(clicker, "message-error")) {
                     String message = this.plugin.getMessage("prefix") + " "
                           + this.plugin.getMessage("game.player-not-found");
                     this.plugin.getAdventureHelper().sendMessage(clicker, message);
                  }
               } else {
                  ...
                  if (this.plugin.getPlayerSettingsManager().isSettingEnabled(clicker, "message-game-cancelled")) {
                     String message = this.plugin.getMessage("prefix") + " "
                           + this.plugin.getMessage("command.bet-cancelled");
                     this.plugin.getAdventureHelper().sendMessage(clicker, message);
                  }
               }
            }
         ...
         if (detector != null && detector.isEnabled()) {
            detector.report(clicker, ExploitDetector.ExploitType.DOUBLE_CLICK_JOIN,
                  "GameId=" + game.getGameId() + ", Amount=" + game.getAmount());
         }
         ...
         if (!gamesBeingJoined.add(gameId)) {
            ...
            if (detectorx != null && detectorx.isEnabled()) {
               detectorx.report(clicker, ExploitDetector.ExploitType.RACE_CONDITION_JOIN,
                     "GameId=" + gameId + ", Amount=" + game.getAmount());
            }
            ...
            try {
               ...
               synchronized (joinGameLock) {
                  ...
                  if (hostPlayer != null && hostPlayer.isOnline()) {
                     if (!this.plugin.getCurrencyManager().canPlayersGambleTogether(hostPlayer, clicker, currencyType,
                           currencyId)) {
                        boolean joinerCanUse = this.plugin.getCurrencyManager().canPlayerUseCurrency(clicker,
                              currencyType, currencyId);
                        String message;
                        if (!joinerCanUse) {
                           String restrictionReason = this.plugin.getCurrencyManager().getRestrictionReason(clicker,
                                 currencyType, currencyId);
                           message = this.plugin.getMessage("prefix") + " "
                                 + this.plugin.getMessage("restriction.cannot-join-game");
                           if (restrictionReason != null && !restrictionReason.isEmpty()) {
                              message = message + " " + restrictionReason;
                           }
                        } else {
                           message = this.plugin.getMessage("prefix") + " "
                                 + this.plugin.getMessage("restriction.host-cannot-use");
                        }
                  ...
                  } else if (!this.plugin.getCurrencyManager().canPlayerUseCurrency(clicker, currencyType,
                        currencyId)) {
                     this.plugin.getSoundHelper().playSound(clicker, "error.general");
                     String restrictionReason = this.plugin.getCurrencyManager().getRestrictionReason(clicker,
                           currencyType, currencyId);
                  ...
                  if (!this.plugin.getCurrencyManager().hasBalanceWithReserve(clicker, currencyType, currencyId,
                        amount)) {
                     if (this.plugin.getPlayerSettingsManager().isSettingEnabled(clicker, "notification-sound")) {
                     ...
                     if (this.plugin.getPlayerSettingsManager().isSettingEnabled(clicker, "message-error")) {
                        double currentBalance = this.plugin.getCurrencyManager().getBalance(clicker, currencyType,
                              currencyId);
                        ...
                        boolean isReserveIssue = this.plugin.getCurrencyManager().isReserveBalanceIssue(clicker,
                  ...
                  if (removedGame == null) {
                     ...
                     if (detectorxx != null && detectorxx.isEnabled()) {
                        detectorxx.report(clicker, ExploitDetector.ExploitType.GAME_ALREADY_TAKEN,
                  ...
                  if (!this.plugin.getCurrencyManager().withdraw(clicker, currencyType, currencyId, amount)) {
                     ...
                     if (detectorxxx != null && detectorxxx.isEnabled()) {
                        detectorxxx.report(
                              ...
                              "GameId=" + gameId + ", Amount=" + amount + ", Currency=" + currencyType + "/"
               ...
               CoinFlipRollGUI rollGUI = new CoinFlipRollGUI(
                     this.plugin, rollGame.getHost(), clicker, rollGame.getAmount(), rollGame.getCurrencyType(),
                     rollGame.getCurrencyId());
               ...
               game -> {
                  if (game == null) {
                     ...
                     return hostPlayer != null && hostPlayer.isOnline()
                           ? !this.plugin.getCurrencyManager().canPlayersGambleTogether(hostPlayer, this.viewer,
                                 currencyType, currencyId)
                           : !this.plugin.getCurrencyManager().canPlayerUseCurrency(this.viewer, currencyType,
                                 currencyId);
                  }
               });
   ...
   private List<CoinFlipGame> applyFilter(List<CoinFlipGame> games) {
      if (games != null && !games.isEmpty()) {
         int gamesVersion = this.plugin.getCoinFlipManager().getGamesListVersion();
         List<CoinFlipGame> sortedGames = this.plugin.getFilteredGamesCache().getFilteredGames(games, this.filterType,
               gamesVersion);
