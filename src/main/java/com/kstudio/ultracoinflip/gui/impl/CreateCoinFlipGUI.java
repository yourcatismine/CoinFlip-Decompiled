...
public class CreateCoinFlipGUI extends InventoryGUI {
   ...
   public void decorate(Player player) {
      if (player != null && player.isOnline()) {
         String fillerMaterialName = this.plugin.getGUIConfig().getString("create-gui.filler.material",
         ...
         List<Integer> fillerSlotsList = this.plugin.getGuiHelper().parseSlotList(this.plugin.getGUIConfig(),
               "create-gui.filler.slots");
         Set<Integer> excludedSlots = new HashSet<>();
         Set<String> decreaseButtonKeys = (Set<String>) (this.plugin.getGUIConfig()
               .getConfigurationSection("create-gui.decrease.buttons") != null
         ...
         Set<String> increaseButtonKeys = (Set<String>) (this.plugin.getGUIConfig()
               .getConfigurationSection("create-gui.increase.buttons") != null
            ...
            if (!this.plugin.getGUIConfig().contains(buttonPath + ".slot")) {
               ...
               if (slotx >= 0 && slotx < this.getInventory().getSize()) {
                  ...
                  this.addButton(slotx, new InventoryButton()
                        .creator(p -> this.createDecreaseButton(finalLevel, finalAmount)).consumer(event -> {
         ...
         for (String buttonKeyxxx : increaseButtonKeys) {
            ...
            if (!this.plugin.getGUIConfig().contains(buttonPath + ".slot")) {
               ...
               if (slotx >= 0 && slotx < this.getInventory().getSize()) {
                  ...
                  this.addButton(slotx, new InventoryButton()
                        .creator(p -> this.createIncreaseButton(finalLevel, finalAmount)).consumer(event -> {
         ...
         this.addButton(
               ...
               new InventoryButton()
                     ...
                     .consumer(
                           event -> {
                              String inputMethod = this.plugin.getConfig().getString("input.method", "CHAT")
                                    .toUpperCase();
                              player.closeInventory();
                              if ("ANVIL".equals(inputMethod)) {
                                 String anvilTitle = this.plugin.getGUIConfig()
                                       .getString("create-gui.custom-amount.anvil.title", "&6Enter Amount");
                                 String initialTextTemplate = this.plugin.getGUIConfig()
                                       .getString("create-gui.custom-amount.anvil.initial-text", "&e<current>");
                                 String placeholder = this.plugin
                                       .getGUIConfig()
                                       .getString("create-gui.custom-amount.anvil.placeholder",
                                             "Enter amount (1K, 1M, 1B, 1T)");
                                 String initialText = "";
                                 if (initialTextTemplate != null && !initialTextTemplate.isEmpty()) {
                                    if (!initialTextTemplate.contains("<current>")) {
                                       ...
                                       if (this.currentAmount == Math.floor(this.currentAmount)) {
                                          ...
                                          plainAmount = plainAmount.replaceAll("\\.0+$", "")
                                 ...
                                 this.plugin.getAnvilInputManager().requestInput(player, parsedTitle, initialText,
                                       input -> {
                                          try {
                                             ...
                                             if (amountxx <= 0.0) {
                                                if (this.plugin.getPlayerSettingsManager().isSettingEnabled(player,
                                                      "notification-sound")) {
                                                   this.plugin.getSoundHelper().playSound(player,
                                                         "error.invalid-amount");
                                                }

                                                if (this.plugin.getPlayerSettingsManager().isSettingEnabled(player,
                                                      "message-error")) {
                                                   String errorMsg = this.plugin.getMessage("prefix")
                                             ...
                                             newGUI.setSelectedCurrency(this.selectedCurrencyType,
                                             ...
                                             if (this.plugin.getPlayerSettingsManager().isSettingEnabled(player,
                                                   "message-error")) {
                                                String errorMsg = this.plugin.getMessage("prefix") + " &c"
                                                      + var6x.getMessage();
                                                this.plugin.getAdventureHelper().sendMessage(player, errorMsg);
                                             }
                                          }
                                 ...
                                 String messageTemplate = this.plugin
                                       .getGUIConfig()
                                       .getString(
                                             "create-gui.custom-amount.chat-message", this.plugin.getMessage("prefix")
                                                   + " &7Enter amount (supports format: 1K, 1M, 1B, 1T):");
                     ...
                     .consumer(
                           event -> {
                              if (this.selectedCurrencyType == null
                                    || !this.plugin.getCurrencyManager().isCurrencyEnabled(this.selectedCurrencyType,
                                          this.selectedCurrencyId)) {
                                 String message = this.plugin.getMessage("prefix")
                                       + " &cPlease select a currency before creating a game!";
                                 this.plugin.getAdventureHelper().sendMessage(player, message);
                              } else if (!this.plugin.getCurrencyManager().canPlayerUseCurrency(player,
                                    this.selectedCurrencyType, this.selectedCurrencyId)) {
                                 String restrictionReason = this.plugin
                                       .getCurrencyManager()
                                       .getRestrictionReason(player, this.selectedCurrencyType,
                                             this.selectedCurrencyId);
                                 String message = this.plugin.getMessage("prefix") + " "
                                 ...
                                 this.plugin.getAdventureHelper().sendMessage(player, message);
                                    ...
                                    .getCurrencyManager()
                                    .hasBalanceWithReserve(player, this.selectedCurrencyType, this.selectedCurrencyId,
                                          this.currentAmount)) {
                                 if (this.plugin.getPlayerSettingsManager().isSettingEnabled(player,
                                       "notification-sound")) {
                                 ...
                                 if (this.plugin.getPlayerSettingsManager().isSettingEnabled(player, "message-error")) {
                                    double currentBalance = this.plugin.getCurrencyManager().getBalance(player,
                                          this.selectedCurrencyType, this.selectedCurrencyId);
                                    String formattedBalance = this.plugin.getGuiHelper().formatAmount(currentBalance,
                                          this.selectedCurrencyId);
                                    String formattedAmount = this.plugin.getGuiHelper().formatAmount(this.currentAmount,
                                          this.selectedCurrencyId);
                                    boolean isReserveIssue = this.plugin
                                          .getCurrencyManager()
                                          .isReserveBalanceIssue(player, this.selectedCurrencyType,
                                                this.selectedCurrencyId, this.currentAmount);
                                    Map<String, String> placeholders = new HashMap<>();
                                    String messageKey;
                                    if (isReserveIssue) {
                                       double minReserve = this.plugin.getCurrencyManager()
                                             .getMinReserveBalance(this.selectedCurrencyType, this.selectedCurrencyId);
                                       double maxBet = currentBalance - minReserve;
                                       String formattedReserve = this.plugin.getGuiHelper().formatAmount(minReserve,
                                             this.selectedCurrencyId);
                                       String formattedMaxBet = this.plugin.getGuiHelper().formatAmount(maxBet,
                                             this.selectedCurrencyId);
   ...
   private ItemStack createDecreaseButton(int level, double amount) {
      ...
      String materialName = this.plugin
            .getGUIConfig()
            .getString(buttonPath + ".material",
                  this.plugin.getGUIConfig().getString("create-gui.decrease.material", "RED_STAINED_GLASS_PANE"));
      ItemStack item = MaterialHelper.createItemStack(materialName);
      if (item == null) {
         item = MaterialHelper.getRedStainedGlassPaneItem();
         if (item == null) {
            item = new ItemStack(
                  MaterialHelper.getRedStainedGlassPane() != null ? MaterialHelper.getRedStainedGlassPane()
                        : Material.GLASS_PANE);
         }
      }
      ...
      String titleTemplate = this.plugin
            .getGUIConfig()
            .getString(buttonPath + ".title",
                  this.plugin.getGUIConfig().getString("create-gui.decrease.title", "&r&c&lDecrease <amount>"));
      Map<String, String> placeholders = new HashMap<>();
   ...
   private ItemStack createIncreaseButton(int level, double amount) {
      ...
      String materialName = this.plugin
            .getGUIConfig()
            .getString(buttonPath + ".material",
                  this.plugin.getGUIConfig().getString("create-gui.increase.material", "LIME_STAINED_GLASS_PANE"));
      ItemStack item = MaterialHelper.createItemStack(materialName);
      if (item == null) {
         item = MaterialHelper.getLimeStainedGlassPaneItem();
         if (item == null) {
            item = new ItemStack(
                  MaterialHelper.getLimeStainedGlassPane() != null ? MaterialHelper.getLimeStainedGlassPane()
                        : Material.GLASS_PANE);
         }
      }
      ...
      String titleTemplate = this.plugin
            .getGUIConfig()
            .getString(buttonPath + ".title",
                  this.plugin.getGUIConfig().getString("create-gui.increase.title", "&r&a&lIncrease <amount>"));
      Map<String, String> placeholders = new HashMap<>();
      String formattedAmount = this.formatAmount(amount);
      placeholders.put("amount", formattedAmount);
      String title = this.plugin.getAdventureHelper().parseToLegacy(titleTemplate, placeholders);
   ...
   private ItemStack createCurrencyButton() {
      ...
      String currencyDisplayName = this.selectedCurrencyType != null
            ...
            : "Not selected";
      String unit = this.selectedCurrencyType != null
            ? this.plugin.getCurrencyManager().getUnit(this.selectedCurrencyType, this.selectedCurrencyId)
            : "";
   ...
   private List<CreateCoinFlipGUI.CurrencyItem> getEnabledCurrencies() {
      ...
      if (this.plugin.getCurrencyManager().isCurrencyEnabled(CoinFlipGame.CurrencyType.MONEY)
            && (this.viewer == null || this.plugin.getCurrencyManager().canPlayerUseCurrency(this.viewer,
                  CoinFlipGame.CurrencyType.MONEY, null))) {
      ...
      if (this.plugin.getCurrencyManager().isCurrencyEnabled(CoinFlipGame.CurrencyType.PLAYERPOINTS)
            && (this.viewer == null || this.plugin.getCurrencyManager().canPlayerUseCurrency(this.viewer,
                  CoinFlipGame.CurrencyType.PLAYERPOINTS, null))) {
      ...
      if (this.plugin.getCurrencyManager().isCurrencyEnabled(CoinFlipGame.CurrencyType.TOKENMANAGER)
            && (this.viewer == null || this.plugin.getCurrencyManager().canPlayerUseCurrency(this.viewer,
                  CoinFlipGame.CurrencyType.TOKENMANAGER, null))) {
      ...
      if (this.plugin.getCurrencyManager().isCurrencyEnabled(CoinFlipGame.CurrencyType.BEASTTOKENS)
            && (this.viewer == null || this.plugin.getCurrencyManager().canPlayerUseCurrency(this.viewer,
                  CoinFlipGame.CurrencyType.BEASTTOKENS, null))) {
      ...
      for (String id : this.plugin.getCurrencyManager().getEnabledCoinsEngineCurrencyIds()) {
         if (this.viewer == null || this.plugin.getCurrencyManager().canPlayerUseCurrency(this.viewer,
               CoinFlipGame.CurrencyType.COINSENGINE, id)) {
      ...
      for (String idx : this.plugin.getCurrencyManager().getEnabledPlaceholderCurrencyIds()) {
         if (this.viewer == null || this.plugin.getCurrencyManager().canPlayerUseCurrency(this.viewer,
               CoinFlipGame.CurrencyType.PLACEHOLDER, idx)) {
   ...
   private ItemStack createCustomAmountButton() {
      ...
      String title = this.plugin.getGUIConfig().getString("create-gui.custom-amount.title",
            "&r&6&lEnter Custom Amount");
   ...
   private ItemStack createCreateButton() {
      ...
      if (this.selectedCurrencyType != null
         ...
         String currencyUnit = this.plugin.getCurrencyManager().getUnit(this.selectedCurrencyType,
               this.selectedCurrencyId);
         String currencyName = this.plugin.getCurrencyManager().getDisplayName(this.selectedCurrencyType,
               this.selectedCurrencyId);
         ...
         if (!this.plugin.getCurrencyManager().hasBalanceWithReserve(this.viewer, this.selectedCurrencyType,
               this.selectedCurrencyId, this.currentAmount)) {
            String materialName = this.plugin.getGUIConfig().getString("create-gui.create.material-insufficient",
                  "BARRIER");
            ...
            String titleTemplate = this.plugin.getGUIConfig().getString("create-gui.create.title-insufficient",
                  "&r&c&lInsufficient Balance");
            ...
            double balance = this.plugin.getCurrencyManager().getBalance(this.viewer, this.selectedCurrencyType,
                  this.selectedCurrencyId);
            ...
            String titleTemplate = this.plugin.getGUIConfig().getString("create-gui.create.title",
                  "&r&a&lCreate CoinFlip");
            title = this.plugin.getAdventureHelper().parseToLegacy(titleTemplate, placeholders);
         ...
         String titleTemplate = this.plugin.getGUIConfig().getString("create-gui.create.title-no-currency",
               "&r&c&lNo Currency Selected");
      ...
      if (this.selectedCurrencyType != null
         ...
         try {
            CurrencySettings currencySettings = this.plugin.getCurrencyManager()
                  .getCurrencySettings(this.selectedCurrencyType, this.selectedCurrencyId);
   ...
   private void createCoinFlip(Player player) {
      if (this.currentAmount <= 0.0) {
         ...
         if (this.currentAmount < minBid) {
            ...
            if (maxBid != -1.0 && this.currentAmount > maxBid) {
            ...
            } else if (!this.plugin.getCurrencyManager().hasBalanceWithReserve(player, this.selectedCurrencyType,
                  this.selectedCurrencyId, this.currentAmount)) {
               if (this.plugin.getPlayerSettingsManager().isSettingEnabled(player, "notification-sound")) {
                  this.plugin.getSoundHelper().playSound(player, "error.not-enough-money");
               }
               ...
               if (this.plugin.getPlayerSettingsManager().isSettingEnabled(player, "message-error")) {
                  double currentBalance = this.plugin.getCurrencyManager().getBalance(player, this.selectedCurrencyType,
                        this.selectedCurrencyId);
                  String formattedBalance = this.plugin.getGuiHelper().formatAmount(currentBalance,
                        this.selectedCurrencyId);
                  String formattedAmount = this.plugin.getGuiHelper().formatAmount(this.currentAmount,
                  ...
                  boolean isReserveIssue = this.plugin
                        .getCurrencyManager()
                        .isReserveBalanceIssue(player, this.selectedCurrencyType, this.selectedCurrencyId,
                              this.currentAmount);
                  Map<String, String> placeholders = new HashMap<>();
                  String messageKey;
                  if (isReserveIssue) {
               ...
               if (limitResult != null) {
                  ...
                  if (this.plugin.getPlayerSettingsManager().isSettingEnabled(player, "message-error")) {
                     ...
                     placeholders.put("limit",
                           this.plugin.getGuiHelper().formatAmount(limitResult.getLimit(), this.selectedCurrencyId));
                     placeholders.put("current", this.plugin.getGuiHelper().formatAmount(limitResult.getCurrentTotal(),
                           this.selectedCurrencyId));
                     placeholders.put("remaining", this.plugin.getGuiHelper().formatAmount(limitResult.getRemaining(),
                           this.selectedCurrencyId));
                     String message = this.plugin.getMessage("prefix") + " "
                  ...
                  if (headsTailsEnabled) {
                                 ...
                                 this.plugin
                                       ...
                                       .openGUI(
                                             new HeadsTailsSelectionGUI(
                                                   this.plugin, player, this.selectedCurrencyType,
                                                   this.selectedCurrencyId, this.currentAmount, () -> {
                                                      if (player != null && player.isOnline()) {
                                                         this.plugin.getGuiManager().openGUI(
                                                               new CreateCoinFlipGUI(this.plugin, player), player);
                                                      }
                                                   }),
                                             player);
                              }
                     ...
                     if (!this.plugin.getCurrencyManager().withdraw(player, this.selectedCurrencyType,
                           this.selectedCurrencyId, this.currentAmount)) {
                     ...
                     this.plugin.getCoinFlipManager().createGame(player, this.selectedCurrencyType,
                           this.selectedCurrencyId, this.currentAmount);
                     this.plugin.getBettingLimitManager().recordBet(player, this.selectedCurrencyType,
                           this.selectedCurrencyId, this.currentAmount);
                     Map<String, String> placeholders = new HashMap<>();
                     String formattedAmount = this.plugin.getGuiHelper().formatAmount(this.currentAmount,
                           this.selectedCurrencyId);
                     placeholders.put("amount", formattedAmount);
                     String unit = this.plugin.getCurrencyManager().getUnit(this.selectedCurrencyType,
                           this.selectedCurrencyId);
                     ...
                     if (broadcastEnabled && this.currentAmount >= minBroadcastAmount) {
                        ...
                        String formattedBroadcastAmount = this.plugin.getGuiHelper().formatAmount(this.currentAmount,
                              this.selectedCurrencyId);
                        ...
                        this.plugin
                              ...
                              .broadcastWithFilter(
                                    broadcastMsg, broadcastPlaceholders, p -> this.plugin.getPlayerSettingsManager()
                                          .isSettingEnabled(p, "message-broadcasts"));
                     }
